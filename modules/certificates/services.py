...
class EligibilityService:
    ...
    @classmethod
    def check_eligibility(cls, cur, student_id, program_id):
        """
        Evaluates student eligibility for a certificate for a given LMS Program.
        ...
        # 1. Fetch Program and mapped Course
        program = cur.execute(
            "SELECT id, program_name, course_id FROM lms_programs WHERE id = ?",
            (program_id,)
        ).fetchone()
        if not program:
            return False, {"error": "LMS Program not found"}, {}

        course_id = program["course_id"]
        if not course_id:
            return False, {"error": "Program is not mapped to any Course"}, {}
        ...
        if not course:
            return False, {"error": "Mapped Course not found"}, {}
        ...
        # 3. Syllabus Progress (100% complete)
        ...
        # 5. Financial Dues (Must be <= 0 balance)
        ...
        is_eligible = has_passed_exam and syllabus_completed and assignments_completed and no_dues
...
class CertificateService:
    ...
    @classmethod
    def issue_certificate(cls, conn, student_id, program_id, grade=None, completion_date=None, notes=None, performed_by=None, ip_address=None, user_agent=None, force=False):
        """
        ...
        program = cur.execute("SELECT course_id FROM lms_programs WHERE id = ?", (program_id,)).fetchone()
        if not program or not program["course_id"]:
            raise ValueError("LMS Program is not associated with a Course")
        ...
        is_eligible, reasons, details = EligibilityService.check_eligibility(cur, student_id, program_id)
