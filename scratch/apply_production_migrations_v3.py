"""Execute production migrations with precise MySQL procedure parsing."""
...
def parse_mysql_file(sql_text):
    ...
    for line in sql_text.splitlines():
        ...
        if delimiter == "//":
            if trimmed.endswith("//"):
                current.append(line[:line.rfind("//")])
                stmt = "\n".join(current).strip()
                if stmt:
...
def run_migrations():
    ...
    try:
        for filename in MIGRATION_FILES:
            ...
            statements = parse_mysql_file(sql_text)
