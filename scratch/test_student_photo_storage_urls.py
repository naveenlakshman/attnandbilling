...
def test_private_legacy_photos_are_served_through_authenticated_app_route():
    assert '@app.get("/student-photos/<path:filename>")' in APP
    assert 'int(authenticated_tenant or 0) != 1' in APP
    assert 'object_path = f"student_photos/{safe_filename}"' in APP
